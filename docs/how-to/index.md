# How-to guides

The following guides cover key processes and common tasks for managing and using the Penpot charm.

## Configure and integrate

Once you've finished setting up the charm, now you can perform a number of actions with your deployment. These guides provide instructions on basic operations you can complete with the charm.

- [Configure S3](configure-s3.md)
- [Configure SMTP](configure-smtp.md)
- [Configure Single Sign-On (SSO)](configure-sso.md)
- [Integrate with COS](integrate-with-cos.md)
- [Manage Penpot users](manage-users.md)

## Update and refresh

The following guides provide instructions on upgrading your deployment, including backup and redeployment processes.

- [Upgrade](upgrade.md)

## Development

These guides can help you with contributing to the project.

- [Contribute](contribute.md)
